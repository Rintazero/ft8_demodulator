...
def estimate_frequency_drift(max_freqs, time_axis, signal_segment, poly_degree=2):
    """
    对检测到的信号段进行频率漂移估计
    
    参数:
    max_freqs: 频谱图每个时间点的最大值频率
    time_axis: 时间轴，单位为秒
    signal_segment: 信号段的起始和结束索引 (start_idx, end_idx)
    poly_degree: 多项式拟合的阶数，默认为2（二次多项式）
    
    返回:
    tuple: (频率漂移率, 频率漂移加速度, 拟合模型)
        - 频率漂移率: 每秒Hz的频率变化率 (Hz/s)
        - 频率漂移加速度: 每秒平方Hz的频率变化加速度 (Hz/s²)
        - 拟合模型: 用于预测频率漂移的多项式模型 (tuple of (poly, model))
    """
    start_idx, end_idx = signal_segment
