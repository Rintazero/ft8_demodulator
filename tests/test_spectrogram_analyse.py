...
sys.path.append('/home/peng/ft8_generator')
...
from ft8_demodulator.ft8_decode import (
    decode_ft8_message,
    FT8Message,
    FT8DecodeStatus,
)

# Import ft8_generator (should be managed through setup.py dependencies)
try:
    import ft8_generator
except ImportError:
    print("Warning: ft8_generator module not found, some tests may fail")
    print("Please ensure ft8_generator is properly installed or in the correct path")
...
def test_decode_edge_cases():
    ...
    test_payload = np.array([0x1C, 0x3F, 0x8A, 0x6A, 0xE2, 0x07, 0xA1, 0xE3, 0x94, 0x51], dtype=np.uint8)
    wave_data = ft8_generator.ft8_generator(test_payload, fs=fs, f0=300, fc=0)
